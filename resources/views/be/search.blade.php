...
@section('content')
    <div class="hud-panel mb-4">
        <div class="font-ui text-gray" style="font-size: 0.82rem;">Kata kunci</div>
    ...
    <div class="qol-search-grid">
        <div class="hud-panel">
            <div class="hud-header mb-4">
                <h3 class="font-bank text-main">Shipment</h3>
            ...
            <div class="qol-result-list">
                ...
                @empty
                    <x-be.empty title="Shipment tidak ditemukan" body="Coba nomor resi lengkap, nama customer, atau nomor HP." />
                @endforelse
            </div>
        </div>
        ...
        <div class="hud-panel">
            <div class="hud-header mb-4">
                <h3 class="font-bank text-main">Pickup</h3>
                ...
                @forelse($pickups as $pickup)
                    <a href="{{ route('be.pickups.show', $pickup) }}" class="qol-result">
                        <div>
                            ...
                            <div class="font-ui text-gray" style="font-size: 0.72rem; margin-top: 0.25rem;">
                                {{ optional($pickup->branch)->name ?? 'Belum ada hub' }}
                            </div>
                        </div>
                        <x-be.badge variant="neutral">{{ strtoupper($pickup->status) }}</x-be.badge>
                    </a>
                @empty
                    <x-be.empty title="Pickup tidak ditemukan" body="Pickup bisa dicari dari nama pengirim, penerima, alamat, atau nomor HP." />
                @endforelse
            </div>
        </div>
        ...
        <div class="hud-panel">
            <div class="hud-header mb-4">
                <h3 class="font-bank text-main">Hub</h3>
            ...
            <div class="qol-result-list">
                @forelse($branches as $branch)
                    ...
                    @endif
                @empty
                    <x-be.empty title="Hub tidak ditemukan" body="Coba nama provinsi, kota, alamat, atau nomor telepon hub." />
                @endforelse
            </div>
        </div>
        ...
        <div class="hud-panel">
            <div class="hud-header mb-4">
                <h3 class="font-bank text-main">Personel</h3>
                ...
                @forelse($users as $person)
                    <a href="{{ route('be.users.index', ['search' => $person->name]) }}" class="qol-result">
                        <div>
                            ...
                            <div class="font-ui text-gray" style="font-size: 0.72rem; margin-top: 0.25rem;">{{ optional($person->branch)->name ?? 'Tanpa hub' }}</div>
                        </div>
                    </a>
                @empty
                    <x-be.empty title="Personel tidak ditemukan" body="Coba nama, email, telepon, atau nama hub." />
                @endforelse
            </div>
        </div>
    </div>
